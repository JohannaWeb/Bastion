...
async fn create_server(
    ...
    let protocol = payload["protocol"].as_str().unwrap_or("activitypub");

    // Insert server
    let server_id = sqlx::query("INSERT INTO servers (name, owner_did, protocol) VALUES (?, ?, ?)")
        .bind(name)
        .bind(owner_did)
        .bind(protocol)
        .execute(&state.db)
...
async fn post_message(
    ...
    let protocol = payload["protocol"].as_str().map(|s| s.to_string());
    let external_id = payload["externalId"].as_str().map(|s| s.to_string());

    let id = sqlx::query(
        "INSERT INTO messages (channel_id, author_did, author_handle, content, protocol, external_id) VALUES (?, ?, ?, ?, ?, ?)",
    )
    .bind(channel_id)
    .bind(author_did)
    .bind(author_handle)
    ...
    // If it's an internal message, we check if we should broadcast to ActivityPub
    if protocol.is_none() {
        ...
        let payload = json!({
            ...
            "targetDid": "did:web:fediverse.peer" // Example target
        });
        
        let _ = client.post("http://localhost:8081/translate/atproto-to-ap")
            .json(&payload)
            .send()
            .await;
            
        tracing::info!("Dispatched message {} to external bridge for broadcast", id);
    }
...
async fn send_convo_message(
    ...
    let msg_id = sqlx::query(
        "INSERT INTO conversation_messages (conversation_id, author_did, author_handle, content, protocol, external_id) VALUES (?, ?, ?, ?, ?, ?)",
    )
    .bind(final_convo_id)
    .bind(author_did)
    .bind(author_handle)
    ...
    // If it's an internal message, check for bridge targets
    if protocol.is_none() {
        let client = reqwest::Client::new();
        if let Ok(res) = client.get(format!("http://localhost:8081/identity/resolve/{}", author_did)).send().await {
            if res.status().is_success() {
                if let Ok(mapping) = res.json::<Value>().await {
                    tracing::info!("Resolved bridge mapping for recipient: {}. Dispatching DM via bridge.", mapping["actorUri"]);
                }
            }
        }
    }
